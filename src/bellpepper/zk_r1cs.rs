...
/// Number of blinding factors added to the witness
...
impl<E: Engine> SpartanShape<E> for ShapeCS<E> {
  fn r1cs_shape<C: SpartanCircuit<E>>(circuit: &C) -> Result<SplitR1CSShape<E>, SpartanError> {
    ...
    // Add vars and constraints for zk blinding
    let mut blinds = vec![];
    for _ in 0..NUM_BLINDS {
      let blind = AllocatedNum::alloc(&mut cs, || Ok(E::Scalar::ZERO)).unwrap();
      blinds.push(blind);
    }
    // blind_0 * 0 = 0
    cs.enforce(
      || "blind_0 * 0 = 0",
      |lc| lc + blinds[0].get_variable(),
      |lc| lc,
      |lc| lc,
    );
    // 0 * blind_1 = 0
    cs.enforce(
      || "0 * blind_1 = 0",
      |lc| lc,
      |lc| lc + blinds[1].get_variable(),
      |lc| lc,
    );
    // blind_2 * 1 = blind_2
    cs.enforce(
      || "blind_2 * 1 = blind_2",
      |lc| lc + blinds[2].get_variable(),
      |lc| lc + <Self as ConstraintSystem<E::Scalar>>::one(),
      |lc| lc + blinds[2].get_variable(),
    );
...
impl<E: Engine> SpartanWitness<E> for SatisfyingAssignment<E> {
  ...
  fn r1cs_instance_and_witness<C: SpartanCircuit<E>>(
    ...
    // Add vars and constraints for zk blinding
    let blinds: Vec<_> = (0..NUM_BLINDS)
      .map(|_| E::Scalar::random(&mut OsRng))
      .collect();
    let mut blind_vars = vec![];
    for blind in blinds.iter() {
    ...
    ps.cs.enforce(
      || "blind_0 * 0 = 0",
      |lc| lc + blind_vars[0].get_variable(),
      |lc| lc,
      |lc| lc,
    );
    // 0 * blind_1 = 0
    ps.cs.enforce(
      || "0 * blind_1 = 0",
      |lc| lc,
      |lc| lc + blind_vars[1].get_variable(),
      |lc| lc,
    );
    // blind_2 * 1 = blind_2
    ps.cs.enforce(
      || "blind_2 * 1 = blind_2",
      |lc| lc + blind_vars[2].get_variable(),
      |lc| lc + <Self as ConstraintSystem<E::Scalar>>::one(),
      |lc| lc + blind_vars[2].get_variable(),
    );
